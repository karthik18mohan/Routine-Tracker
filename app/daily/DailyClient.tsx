...
export default function DailyClient() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const json = (await response.json()) as DailyResponse;
      setData(json);
      setDirtyAnswers({});
      setSaving(false);
    } catch {
  ...
  useEffect(() => {
    ...
    // only depends on searchParams, it updates when query changes
  }, [searchParams]);
  ...
  const loadEditQuestions = useCallback(async () => {
    ...
    try {
      ...
      setEditQuestions((json.questions ?? []) as Question[]);
    } catch {
  ...
  const saveAnswers = useCallback(async () => {
    ...
    if (failed.length) pushToast("Unable to save some answers.", "error");
    setSaving(false);
  }, [data, date, dirtyAnswers, pushToast]);

  useEffect(() => {
    const hasDirty = Object.keys(dirtyAnswers).some((id) => dirtyAnswers[id]);
    if (!hasDirty || saving) return;

    if (autoSaveTimer.current) clearTimeout(autoSaveTimer.current);
    autoSaveTimer.current = setTimeout(() => void saveAnswers(), 800);
                  ...
                  <section key={section.id} className="space-y-4">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      ...
                      <button
                        ...
                        onClick={async () => {
                          const current = editQuestions.filter((q) => q.section_id === section.id);
                          const nextSort =
                            current.length === 0 ? 0 : Math.max(...current.map((q) => q.sort_order)) + 1;
                  ...
                  <section key={section.id} className="space-y-4">
                      ...
                      {(orderedQuestionsBySection[section.id] ?? []).map((question) => (
                        <div key={question.id} className="space-y-1">
                          <QuestionRenderer
                            ...
                            onChange={(v) => updateAnswer(question, v)}
                          />
                        </div>
                      ))}
                    </div>
                    ...
                    {section.key === "routine" && (
                      <div className="grid gap-6 lg:grid-cols-2">
                        ...
                        <div className="space-y-3">
                          <h3 className="text-lg font-semibold text-slate-900">Plan tomorrow</h3>
